...
from .models import Beneficiario, MilitarIncluso, FinalizadoMilitar, FinalizadoBeneficiario, TB_Vencimentos, MemorialDeCalculo, OutrosMemorial, FinalizadoMilitar, FinalizadoBeneficiario, BeneficiarioMemorial
...
class MilitarDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['memoriais'] = MemorialDeCalculo.objects.filter(militar=militar)
...
def militar_detail(request, pk):
    ...
    memoriais = MemorialDeCalculo.objects.filter(militar=militar)  # Filtra memoriais pelo militar

    context = {
        'militar': militar,
        'memoriais': memoriais,
        'beneficiarios': militar.beneficiario_set.all(),  # Supondo que há um relacionamento
    }
...
def gerar_memorial(request, pk):
    militar = get_object_or_404(MilitarIncluso, pk=pk)

    if request.method == "POST":
        form = MemorialDeCalculoForm(request.POST)
        if form.is_valid():
            memorial = form.save(commit=False)
            memorial.militar = militar

            # Formatar o nome do memorial
            data_atual = datetime.now().strftime("%d/%m/%Y")
            memorial.nome = f"{data_atual}_{memorial.protocolo_memorial}"

            memorial.save()

            # Copiar beneficiários para o memorial
            beneficiarios = Beneficiario.objects.filter(militar=militar)
            for ben in beneficiarios:
                BeneficiarioMemorial.objects.create(
                    memorial=memorial,
                    nome=ben.nome,
                    cpf=ben.cpf,
                    cota=ben.cota,
                    agencia_cc=ben.agencia_cc,
                    ir=0,
                    contribuicao_previdencia=0,
                    cpm=0,
                    auxilio_liquido=0
                )
        ...
        form = MemorialDeCalculoForm()

    html_form = render_to_string("memorial_form.html", {"form": form, "militar": militar}, request=request)
    return JsonResponse({"html_form": html_form})

def salvar_memorial_pdf(request, pk):
    militar = get_object_or_404(MilitarIncluso, pk=pk)  # Corrigido para MilitarIncluso
    html_string = render_to_string("memorial_form.html", {"militar": militar})

    # Crie uma resposta HTTP para enviar o PDF gerado
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="memorial_{militar.RE}.pdf"'

    # Converta o HTML para PDF
    pisa_status = pisa.CreatePDF(html_string, dest=response)

    # Verifique se a conversão teve sucesso
    if pisa_status.err:
...
class EditarMemorialView(UpdateView):
    ...
    def get_success_url(self):
        # Redireciona para a página do militar após a edição
        return reverse('militares:militar_detail', kwargs={'pk': self.object.militar.pk})

class DeletarMemorialView(DeleteView):
    model = MemorialDeCalculo
    template_name = 'confirmar_deletar_memorial.html'

    def get_success_url(self):
        # Redireciona para a página do militar após a exclusão
        return reverse('militares:militar_detail', kwargs={'pk': self.object.militar.pk})
